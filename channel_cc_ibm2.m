...
nsolid = 1; % number of completely solid cells on top and bottom of channel
...
celltype(:,1:nsolid) = 0;
celltype(:,(ny-nsolid+1):ny+1) = 0;

% cutcells
celltype(:,nsolid+1) = cfa_area_fac;
celltype(:,ny-nsolid) = cfa_area_fac;
...
% finite volume definitions

VC = dx*dy*ones(nx,ny); % cell volume
AX = dy*ones(nx+1,ny); % face area normal to x
AY = dx*ones(nx,ny+1); % face area normal to y
DYN = dy*ones(1,ny+1); % dy between cell centers in y (used for pressure gradient in v equation)

% adjust for cutcells
for i=1:nx
    for j=1:ny
        if celltype(i,j)>0 & celltype(i,j)<1
            AX(i,j) = dy*cfa_area_fac;
            AX(i+1,j) = dy*cfa_area_fac;
            VC(i,j) = AX(i,j)*AY(i,j);
            if j<floor(ny/2)
                AX(i,j-1)   = dy + dy*(1-cfa_area_fac);
                AX(i+1,j-1) = dy + dy*(1-cfa_area_fac);
                VC(i,j-1) = AX(i,j-1)*AY(i,j-1);
                DYN(j) = 0.5*(AX(i,j-1)+AX(i,j));
                DYN(j+1) = 0.5*(AX(i,j)+AX(i,j+1));
            else
                AX(i,j+1)   = dy + dy*(1-cfa_area_fac);
                AX(i+1,j+1) = dy + dy*(1-cfa_area_fac);
                VC(i,j+1) = AX(i,j+1)*AY(i,j+1);
...
while t<T
    ...
    for j=1:ny
        FZX(1,j)=Z0(j)*U0(j)/(AX(1,j)/dy);
    end
    for j=1:ny
        for i=2:nx+1
            cfa_vel = u(i,j)/(AX(i,j)/dy);
            if cfa_vel>=0
    ...
    for j=1:ny
        for i=1:nx
            Z(i,j) = Z(i,j) - dt*( FZX(i+1,j)*AX(i+1,j)-FZX(i,j)*AX(i,j) + FZY(i,j+1)*AY(i,j+1)-FZY(i,j)*AY(i,j) )/VC(i,j);
        end
    end
    ...
    for j = 2:ny
        for i = 1:nx
            ...
            if faceytype(i,j)==0
                Fv_visc = 0;
                Fy = (0 - v(i,j))/dt - ( H(i,j)-H(i,j-1) )/DYN(j);
            end
    ...
    for j = 1:ny
        for i = 1:nx
            b(i,j) = ( ( u_hat(i+1,j)/(AX(i+1,j)/dy)-u_hat(i,j)/(AX(i,j)/dy) )/dx + ( v_hat(i,j+1)*(DYN(j+1)/dy)-v_hat(i,j)*(DYN(j)/dy) )/dy ) / dt;
        end
    ...
    for j = 1:ny
        for i = 2:nx
            u(i,j) = u_hat(i,j) - dt*( H(i,j)-H(i-1,j) )/dx * (AX(i,j)/dy);
        end
        % apply inflow bc
        u(1,j) = u_hat(1,j);
        % apply outflow bc
        ...
        u(nx+1,j) = u_hat(nx+1,j) - dt*( HNXP1(j)-H(nx,j) )/dx * (AX(nx+1,j)/dy);
    end
    for j = 2:ny
        for i = 1:nx
            v(i,j) = v_hat(i,j) - dt*( H(i,j)-H(i,j-1) )/DYN(j);
        end
    end
    ...
    for j = 1:ny
        for i = 1:nx
            b(i,j) = ( u(i+1,j)/(AX(i+1,j)/dy)-u(i,j)/(AX(i,j)/dy) )/dx + ( v(i,j+1)*(DYN(j+1)/dy)-v(i,j)*(DYN(j)/dy) )/dy;
        end
    end
    % u
    % b
    ...
    for j = 1:ny
        for i = 1:nx
            up(i,j) = 0.5*( u(i+1,j)/(AX(i+1,j)/dy)+u(i,j)/(AX(i,j)/dy) );
